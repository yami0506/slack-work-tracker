...
- Slackで `/work` と入力すると、現在の状態に合う作業開始または作業終了ボタンがモーダルで表示されます。
- 任意で、チーム用チャンネルに常設の作業ボタンを投稿できます。
- 常設パネルには、現在作業中のメンバーと本日の作業時間が表示されます。
- SlackアプリのApp Homeを開くと、自分の状態、現在作業中、本日合計、今週合計、直近5件の作業を確認できます。
- `/work` とApp Homeには、その人の現在状態に合う操作だけが表示されます。
- `▶ 作業開始` を押すと、Supabaseに開始時刻を保存します。
- `■ 作業終了` を押すと、未終了セッションを終了し、作業時間を分単位で保存します。
- 作業開始・終了の成功メッセージは、共有通知が有効ならパネルのスレッドに表示されます。`/work` から操作した場合は本人用モーダルでも確認できます。
...
常設パネルには、現在作業中のメンバー、開始時刻、本日の作業時間も表示されます。
本日の作業時間には、完了済みの作業時間に加えて、作業中セッションの経過時間も含まれます。

### 使い方

メンバーはチャンネル上の `▶ 作業開始` / `■ 作業終了` を押すだけです。
作業開始・終了の成功メッセージは、共有通知が有効ならパネルのスレッドに投稿されます。
`/work` から開くモーダルには、待機中なら `▶ 作業開始`、作業中なら `■ 作業終了` だけが表示されます。成功結果も同じモーダルで確認できます。

App Homeも現在状態に合わせて操作ボタンが切り替わります。完了した作業は新しい順に直近5件まで表示されます。
...
この設定が有効な場合、作業開始・終了の成功メッセージはパネルのスレッドに投稿されます。
エラーや「すでに作業中です」のような個人向けの案内は、本人だけにモーダルで表示されます。
...
15. App Homeの操作ボタンが現在状態に合わせて切り替わることを確認します。
16. App Homeに直近の完了作業が表示されることを確認します。
17. App Homeの `↻ 更新` ボタンで表示が更新されることを確認します。
18. `PUBLIC_ACTIVITY_NOTIFICATIONS=true` の場合は、開始・終了通知が常設パネルのスレッドにまとまることを確認します。
...
時間計算とSlack表示を確認したい場合は、以下を実行します。
